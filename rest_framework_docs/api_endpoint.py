...
class ApiEndpoint(object):
    ...
    def __init__(self, pattern, parent_pattern=None):
        ...
        self.name_parent = simplify_regex(parent_pattern.pattern.regex.pattern).strip('/').replace("\\",'') if parent_pattern else None
    ...
    def __get_path__(self, parent_pattern):
        if parent_pattern:
            return "/{0}{1}".format(self.name_parent.replace("\\",''), simplify_regex(self.pattern.pattern.regex.pattern).replace("\\",''))
        return simplify_regex(self.pattern.pattern.regex.pattern).replace("\\",'')
